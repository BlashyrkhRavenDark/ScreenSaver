...
- **Run / test**: build with the `build` skill, i.e. `dotnet build ScreenSaver.sln -c Release` then three publishes (StreamDeck self-contained `win-x64`, ScreenSaver framework-dependent, Launcher framework-dependent `win-x64`); run full-screen with `ScreenSaver.exe /s` (`/p <hwnd>` preview, `/c` settings). There is no test project: verify on-screen with a `CopyFromScreen` capture (a black 4K frame is roughly 17 KB, a real mosaic is several MB) plus `%LOCALAPPDATA%\ScreenSaver\screensaver.log`.
